...
class CaliforniaPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('california.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `california`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM california WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO california (date, title, link, content)"
